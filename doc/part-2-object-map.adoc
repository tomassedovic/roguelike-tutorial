...
We will need to import the `Color` struct directly as well:
...
This is necessary, because the `Object` code uses `Color` directly, as
opposed to `colors::Color`. We could have kept using the longer name,
but `Color` is a common struct that we'll be using quite often so
we'll save ourselves some typing.

The `self` keyword in the `use` statement means to use the module
itself as well. If we wrote `use tcod::colors::Color`, that would make
`Color` available, but not the `colors` module. The `self` item stands
for `colors` here. It's a shortcut for typing this:
...
fn handle_keys(root: &mut Root, player: &mut Object, map: &Map) -> bool {  // <1>
