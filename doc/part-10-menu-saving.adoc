...
The first line (`json::encode(&some_data)`) takes the data we want to
save (objects and the game state in our case) and turns it to a
{json}[JSON]-encoded String.

That functionality comes from the {rustc-serialize}[rustc-serialize
crate] so we need to add it to our `[dependencies]` in the
`Cargo.toml` file and we'll also enable the "rustc-serialize" feature
in the _tcod_ crate:

[source,toml]
----
[dependencies]
tcod = { version = "0.9", features = ["rustc-serialize"] }
rand = "0.3"
rustc-serialize = "0.3"
----

The `rustc-serialize` crate gives us options to convert our structs
...
src/bin/part-10-menu-saving.rs:1137:28: 1137:40 error: the trait `rustc_serialize::serialize::Encodable` is not implemented for the type `Game` [E0277]
src/bin/part-10-menu-saving.rs:1137     let save_data = try! { json::encode(&(objects, game)) };
                                                               ^~~~~~~~~~~~
src/bin/part-10-menu-saving.rs:1137:21: 1137:60 note: in this expansion of try! (defined in <std macros>)
src/bin/part-10-menu-saving.rs:1137:28: 1137:40 help: run `rustc --explain E0277` to see a detailed explanation
src/bin/part-10-menu-saving.rs:1137:28: 1137:40 note: required by `rustc_serialize::json::encode`
----

:encodable: https://doc.rust-lang.org/rustc-serialize/rustc_serialize/trait.Encodable.html

Apparently, we need to implement {encodable}[the Encodable trait].
...
Luckily, we can just use `#[derive(RustcEncodable)]` and have Rust do
it for us!

[source]
----
#[derive(RustcEncodable)]  // <1>
struct Game {
    map: Map,
    log: Messages,
    inventory: Vec<Object>,
...
shifted from `Game` to `Object`. We'll need to derive `RustcEncodable`
...
To make things compile, you'll have to derive `RustcDecodable` to
every struct and enum that has the `RustcEncodable` trait, too.
...
`Err(rustc_serialize::json::EncoderError)` on failure. `File::create`
and the `write_all` method work similarly although with different
