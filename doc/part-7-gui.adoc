...
tag::monster_death_message[]
    ...
    game.messages.add(messages, format!("{} is dead!", monster.name), ORANGE);  // <4>
...
let player_action = handle_keys(key, &mut tcod, &game, &mut objects);  // <8>
...
<5> Added `game` to the function's arguments
<6> Passing `game` to the function
<7> Passing `game` to the function call
<8> Passing `key` to the function call
